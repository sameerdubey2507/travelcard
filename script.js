const data = [
    {
        ...
        speciality: 'Backcountry skiing and mountain hiking.',
        address: 'Prättigau, Graubünden, Switzerland'
    },
    {
        place: 'Japan Alps',
        title: 'NANGANO',
        ...
        speciality: 'Olympic-grade ski resorts and historic temples.',
    ...
    },
    {
        ...
        speciality: 'Camel treks and nomadic desert culture.',
        address: 'Drâa-Tafilalet Region, Morocco'
    },
    {
        ...
        speciality: 'Giant Sequoias and El Capitan rock climbing.',
    ...
    },
    {
        ...
        speciality: 'World-class kitesurfing and windsurfing.',
        address: 'Tarifa, Province of Cádiz, Spain'
    },
    {
        ...
        speciality: 'Hot air ballooning and cave dwellings.',
        address: 'Nevşehir Province, Cappadocia, Turkey'
    },
]
...
// Event Listeners for Save and Discover
document.querySelectorAll('.bookmark').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (isSaved) {
            svg.style.color = '#fff';
            btn.style.backgroundColor = '#4caf50'; // Green for saved
            // Temporary "Saved" feedback
            const feedback = document.createElement('div');
            feedback.textContent = 'Saved!';
            feedback.className = 'save-feedback';
            btn.appendChild(feedback);
            gsap.to(feedback, { opacity: 1, y: -40, duration: 0.5, onComplete: () => {
                gsap.to(feedback, { opacity: 0, delay: 1, onComplete: () => feedback.remove() });
            }});
        } else {
            btn.style.backgroundColor = '#ecad29';
        }
    });
});
...
document.querySelectorAll('.discover').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        modal.querySelector('.modal-place').textContent = currentData.place;
        modal.querySelector('.modal-title').textContent = `${currentData.title} ${currentData.title2}`;
        modal.querySelector('.modal-desc').textContent = currentData.description;
        modal.querySelector('.modal-speciality').textContent = currentData.speciality;
        modal.querySelector('.modal-address').textContent = currentData.address;
        
        gsap.to(modal, { display: 'flex', opacity: 1, scale: 1, duration: 0.6, ease: 'power4.out' });
        gsap.from('.modal-content > *', { y: 30, opacity: 0, stagger: 0.1, delay: 0.2, duration: 0.6 });
    });
});

document.querySelector('.discover-modal')?.addEventListener('click', () => {
    gsap.to('.discover-modal', { opacity: 0, scale: 0.9, duration: 0.4, onComplete: () => {
        document.querySelector('.discover-modal').style.display = 'none';
    }});
});
